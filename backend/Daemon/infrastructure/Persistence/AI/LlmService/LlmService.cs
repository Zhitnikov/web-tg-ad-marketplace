...
namespace Daemon.infrastructure.Persistence.AI.LlmService
{
    public class LlmService
    {
        ...
        {
            var prompt = $"""Ты профессиональный копирайтер с многолетним опытом создания продающих текстов для Telegram-каналов. Твоя задача - создать максимально убедительный и качественный рекламный пост.
            ...
            for (int attempt = 1; attempt <= maxRetries; attempt++)
            {
                try
                {
                    ...
                    if (!modelResponse.IsSuccessStatusCode)
                    {
                        ...
                        Console.WriteLine($"Ollama error (attempt {attempt}/{maxRetries}): {modelResponse.StatusCode} — {error}");
                    ...
                    if (string.IsNullOrWhiteSpace(response) || response.Length < 20)
                    {
                        if (attempt < maxRetries)
                        {
                            Console.WriteLine($"Generated text too short (attempt {attempt}/{maxRetries}), retrying...");
                            await Task.Delay(1000 * attempt);
                            continue;
                        }
                    }
